...
const AllTasks = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            let tlist = [];
            try {
                const q = query(collection(db, "Assignments"), orderBy('number', "desc"));
                                ...
                                <div class="container py-3 h-100">
                                    <div class="row d-flex justify-content-center align-items-center h-100">
                                        <div class="col col-xl-10">
                                            ...
                                            {tasks.map((task) => (
                                                <div class="card mb-5" style={{ borderRadius: "15px" }} key={task.id}>

                                                    <div class="card-body p-4">
                                                        <h3 class="mb-3">
                                                            Task  {task.number}<span class="mx-2">|</span>
                                                            {task.title}</h3>
                                                        <p class="small mb-0"><i class="fa fa-star fa-lg"></i> <span class="mx-2">|</span> Technology Used
                                                            <strong> </strong>{task.technology}</p>
                                                        <hr class="my-4" />
                                                        <p> {task.brief}</p>
                                                ...
                                                <a href="#!">
                                                    <img src="https://mdbcdn.b-cdn.net/img/Photos/Avatars/avatar-2.webp" alt="avatar"
                                                        class="img-fluid rounded-circle me-3" width="35" />
                                                            ...
                                                            <a href={task.details} type="button" class="btn  btn-primary btn-floating text-light mb-1 w-100 text-light"  download >
                                                                <i class="fa fa-plus mr-2"></i>See More
                                                            </a>
                                                        </div>
                                                    </div>
                                                </div>
